...
namespace Vlindos.Web.Tools
{
    ...
    public static class TypeMerger
    {
        ...
        {
            return pdc.Select(pd => typeBuilder.DefineField(String.Format("_{0}", pd.Name), pd.PropertyType, FieldAttributes.Private)).ToArray();
        }
        ...
        {
            //build properties
            foreach (FieldBuilder t in fields)
            {
//remove '_' from name for public property name
                String propertyName = t.Name.Substring(1);

                //define the property
                PropertyBuilder property = typeBuilder.DefineProperty(propertyName,
                                                                      PropertyAttributes.None, t.FieldType, null);
